            ...
            HStack(spacing: 0) {
                Button(action: {
                    withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) {
                        self.detailWindowType = .profile
                    }
                }) {
                    Text("プロフィール")
                        .font(.system(size: 14, weight: .bold))
                        .foregroundStyle(detailWindowType == .profile ? .white : .white.opacity(0.6))
                        .padding(.horizontal, 20)
                        .padding(.vertical, 10)
                        .background {
                            if detailWindowType == .profile {
                                Capsule()
                ...
                Button(action: {
                    withAnimation(.spring(response: 0.4, dampingFraction: 0.4)) {
                        self.detailWindowType = .location
                    }
                }) {
                    Text("ロケーション")
                        .font(.system(size: 14, weight: .bold))
                        .foregroundStyle(detailWindowType == .location ? .white : .white.opacity(0.6))
                        .padding(.horizontal, 20)
                        .padding(.vertical, 10)
                        .background {
                            if detailWindowType == .location {
                                Capsule()
                                    .fill(Color.mainColor)
                                    .matchedGeometryEffect(id: "tab", in: namespace)
                            }
