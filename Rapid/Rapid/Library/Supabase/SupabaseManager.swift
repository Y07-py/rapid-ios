...
extension SupabaseManager {
    ...
    public func fetchUserWithProfile(userId: UUID) async throws -> RapidUserWithProfile {
        ...
        let profileURLs = try await self.getPresignURLFromStorage(
            bucket: "profile",
            folder: "users/\(user.id.uuidString.lowercased())"
        )
        let profileImages = profileURLs.map { (key, value) -> UserProfileImage in
...
extension SupabaseManager {
    ...
    public func deleteProfileImages(deleteImage: [UserProfileImage]) async throws {
        var storage_paths = deleteImage.map({ "profile/users/\($0.id.uuidString.lowercased()).jpg" })
        try await self.client
            .from("profile_images")
            .delete()
            .in("storage_path", values: storage_paths)
            .execute()
        storage_paths = storage_paths.map({ $0.replacingOccurrences(of: "profile/", with: "") })
        try await self.client.storage
            .from("profile")
            .remove(paths: storage_paths)
    }
}
