    ...
    public init() {
        ...
        NotificationCenter.default.addObserver(forName: .receiveMessageNotification, object: nil, queue: .main) { notification in
            if let userId = notification.userInfo?["user_id"] as? String {
                ...
                Task { @MainActor in
                    do {
                        let user = try await self.supabase.fetchUser(id: UUID(uuidString: userId))
                        let imageUrls = try await self.supabase.getPresignURLFromStorage(
                            bucket: "profile",
                            folder: "users/\(user.id.uuidString.lowercased())")
                        let profileImages = imageUrls.compactMap { (key, value) -> UserProfileImage? in
                        ...
                        let rapidUser = RapidUserWithProfile(user: user, profileImages: profileImages)
        ...
        NotificationCenter.default.addObserver(forName: .receiveLikedVoiceChatRoomNotification, object: nil, queue: .main) { _ in
            Task { @MainActor in
                await self.fetchLikers()
            }
        }
        ...
        do {
            ...
            let likers: [Liker] = try await withThrowingTaskGroup(of: Liker.self) { [weak self] group in
                ...
                for like in filteredLikePairs {
                    group.addTask {
                        let profileImageURLs: [String: URL] = try await self.supabase.getPresignURLFromStorage(
                            bucket: "profile",
                            folder: "users/\(like.fromUserId.uuidString.lowercased())")
                        let profileImages = profileImageURLs.compactMap { (key, value) -> UserProfileImage? in
                            let uuidString = key.replacingOccurrences(of: ".jpg", with: "")
                                .replacingOccurrences(of: ".jpeg", with: "")
                                .replacingOccurrences(of: ".png", with: "")
                            let uuid = UUID(uuidString: uuidString) ?? .init()
                        ...
                        let user = try await self.supabase.fetchUser(id: like.fromUserId)
                ...
                try await withThrowingTaskGroup(of: (ChatRoomWithRecruitment?, Int, SupabaseSubscription?).self) { group in
                    ...
                    for rec in recruitments {
                        ...
                        group.addTask {
                            ...
                            let profileImageURLs: [String: URL] = try await self.supabase.getPresignURLFromStorage(
                                bucket: "profile",
                                folder: "users/\(roomUserId.uuidString.lowercased())")
                            let profileImages = profileImageURLs.compactMap { (key, value) -> UserProfileImage? in
                                let uuidString = key.replacingOccurrences(of: ".jpg", with: "")
                                    .replacingOccurrences(of: ".jpeg", with: "")
                                    .replacingOccurrences(of: ".png", with: "")
                            ...
                            // User profile
                            let roomUser = try await self.supabase.fetchUser(id: roomUserId)
                            let roomUserWithProfile = RapidUserWithProfile(user: roomUser, profileImages: profileImages)
