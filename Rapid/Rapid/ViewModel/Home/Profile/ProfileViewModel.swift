...
extension ProfileViewModel {
    ...
    private func fetchUserProfile() async {
        ...
        do {
            ...
            let profileURLs = try await supabase.getPresignURLFromStorage(
                bucket: "profile",
                folder: "users/\(session.user.id.uuidString.lowercased())"
            )
            SDWebImagePrefetcher.shared.prefetchURLs(profileURLs.compactMap({ $0.value }))
            var profileImages = profileURLs.compactMap { (key, value) -> UserProfileImage? in
                let uuidString = key.replacingOccurrences(of: ".jpg", with: "")
                    .replacingOccurrences(of: ".jpeg", with: "")
                    .replacingOccurrences(of: ".png", with: "")
                let uuid = UUID(uuidString: uuidString) ?? .init()
                return UserProfileImage(imageURL: value, id: uuid)
            }
            
            // Fetch under review images
            let underReviewImages = await fetchUnderReviewImages()
            profileImages.append(contentsOf: underReviewImages)
            
            
            self.user = .init(user: rapidUser, profileImages: profileImages)
...
extension ProfileViewModel {
    ...
    func syncProfileImagesWithServer() {
        ...
        // We want to keep userProfileImages in sync with user.profileImages
        // but preserve items that are currently being uploaded or under review.
        var updatedList: [UserProfileImage] = []
        
        for (idx, img) in self.userProfileImages.enumerated() {
            // 1. If this image is already known to be on the server, use the SERVER version 
            //    to reflect the latest review status and URLs.
            if let serverImg = user.profileImages.first(where: { $0.id == img.id }) {
                var updatedImg = serverImg
                // Preserve local UIImage data ONLY if we are still under review for smooth performance
                if serverImg.isUnderReview {
                    updatedImg.image = img.image
                }
                updatedList.append(updatedImg)
                continue
            }
            
            // 2. If this is a new image currently uploading or under review (has local data) 
            //    but not yet acknowledged by the server's primary storage/metadata API.
            if img.image != nil || img.isUnderReview {
                updatedList.append(img)
                continue
            }
            
            // 3. Otherwise, if we have a stable original image for this slot, revert to it.
            if idx < user.profileImages.count {
                updatedList.append(user.profileImages[idx])
            }
        }
        
        self.userProfileImages = updatedList
    }
    ...
    public func uploadProfileImages() {
        Task {
            ...
            // Only iterate through what the user sees (excluding marked for deletion) 
            // to correctly identify what's being replaced in which slot.
            let effectiveImages = userProfileImages.enumerated().filter { (_, img) in
                !imagesToBeDeleted.contains(where: { $0.value.id == img.id })
            }
            
            for (displayIdx, (_, image)) in effectiveImages.enumerated() {
                // Metadata and TUS upload are ONLY needed for brand new images (have raw data)
                guard let _ = image.image else { continue }
                
                // Which original image is being replaced at this slot?
                var oldImage: UserProfileImage? = nil
                if displayIdx < user.profileImages.count {
                    oldImage = user.profileImages[displayIdx]
                }
            ...
            // Delete images that are in imagesToBeDeleted but NOT being replaced by a new upload
            let trulyDeletedImages = imagesToBeDeleted.filter { (key, _) in
                // Only delete if it existed on server (key < count) 
                // AND it's not being replaced in this upload batch
                user.profileImages.indices.contains(key) && 
                !uploadProfileImageList.contains(where: { $0.imageIndex == key })
            }
            
            if !trulyDeletedImages.isEmpty {
                let trulyDeletedImagesList = trulyDeletedImages.map(\.value)
                try? await self.supabase.deleteProfileImages(deleteImage: trulyDeletedImagesList)
            }
        
            // If no images have changed or being uploaded, we are done
            if uploadProfileImageList.isEmpty { return }
    ...
    public func fetchMessages() {
        Task {
            do {
                ...
                var processedMessages: [NotificationMessage] = []
                ...
                if offset == 0 {
                    self.notificationMessages = processedMessages
                } else {
                    self.notificationMessages.append(contentsOf: processedMessages)
                }
                
                let unreadCount = self.notificationMessages.count(where: { !$0.isRead })
                self.unReadNotoficationMessageCount = unreadCount
                self.currentNotificationMessageOffset = self.notificationMessages.count
            } catch let error {
