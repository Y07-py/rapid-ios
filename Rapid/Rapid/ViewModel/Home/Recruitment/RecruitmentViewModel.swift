    ...
    private func processRecruitmentObjects(_ objects: [RecruitmentWithLike]) async throws -> [RecruitmentWithUserProfile] {
        return try await withThrowingTaskGroup(of: RecruitmentWithUserProfile.self) { [weak self] group in
            ...
            for recruitment in objects {
                group.addTask {
                    let user = try await self.supabase.fetchUser(id: recruitment.recruitmentWithRelations.userId)
                    let profileURLs = try await self.supabase.getPresignURLFromStorage(
                        bucket: "profile",
                        folder: "users/\(user.id.uuidString.lowercased())"
                    )
                    let profileImages = profileURLs.compactMap { (key, value) -> UserProfileImage? in
